...
/**
 * for users wanting an exception thrown if a naive comparison
 * had given the wrong answer. FIXME - put these in separate header files,
 * so it is possible to indirectly include as little as possible (iostream,
 * cassert, stdexcept, cstdlib)
 */
